...
@section('content')
    ...
    @endif
    <form action="{{ route('admin.editProvostData') }}" method="POST">
        @csrf
        <div class="mt-2 space-y-2 px-[8px] lg:px-0">
            <input type="text" name="Hall_Name" value="{{ $data->name }}" hidden>
                ...
                <select name="Department" id=""
                    ...
                    @foreach ($dept as $item)
                        <option value="{{ $item->department }}"
                            @if ($data->dept == $item->department) {{ 'selected' }} @endif>{{ $item->department }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex flex-col justify-start lg:w-full lg:flex-row lg:justify-around">
        ...
        <div class="mt-5 text-center">
            <input type="submit" value="Update Provost"
