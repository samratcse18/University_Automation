...
@section('content')
    <div class="mt-3 space-y-2">
        <a href="{{ route('admin.addSpecialDegree') }}" class="mx-[8px] bg-[#006666] px-4 text-[18px] text-white lg:mx-0 lg:text-[25px]">Add Degree</a>
        <div class="mt-2 space-y-2 px-[8px] lg:px-0">
            <table class="border-2 border-[#006666] w-full">
                <thead>
                    <tr class="border-2 border-[#006666]">
                        <th class="text-center">Professional Degree</th>
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr class="border-2 border-[#006666] text-[12px]">
                            <td class="border-2 border-[#006666] text-center font-semibold uppercase">{{ $item->special_degree }}
                            </td>
                            ...
                            <td class="border-0 border-[#006666] text-center flex justify-around">
                                <form action="{{ route('admin.editSpecialDegree') }}" method="post">
                                    @csrf
    ...
    <script>
        function del(e) {
            ...
            if (result) {
                axios.post('{{ route('admin.deleteSpecialDegree') }}', {
                        "id": e
                    })
                    .then((res) => {
                        location.reload();
