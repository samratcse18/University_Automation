...
@section('content')
    <div class="mt-3 space-y-2">
        <a href="{{ route('admin.addTeacher') }}"
            class="mx-[8px] bg-[#006666] px-4 text-[18px] text-white lg:mx-0 lg:text-[25px]">Add Teacher</a>
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr class="border-2 border-[#006666]">
                            ...
                            <td class="flex justify-around border-0 border-[#006666] px-[6px] text-center lg:px-0">
                                <form action="{{ route('admin.editTeacher') }}" method="POST">
                                    @csrf
    ...
    <script>
        function del(e) {
            ...
            if (result) {
                axios.post('{{ route('admin.teacherDelete') }}', {
                        "id": e
                    })
                    .then((res) => {
                        location.reload();
