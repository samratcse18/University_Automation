...
@section('content')
    ...
    @endif
    <form action="{{ route('admin.editSpecialDegreeData') }}" method="post">
        @csrf
        ...
        <div class="mt-2 space-y-2 px-[8px] lg:px-0">
            ...
            <div class="flex flex-col justify-start lg:flex-row lg:justify-around lg:w-full">
                ...
                <select name="Degree_Level" id=""
                    ...
                    <option value="Under Graduation" @if ($data->class_level == 'Under Graduation') {{ 'selected' }} @endif>Under Graduation</option>
                    <option value="Post Graduation" @if ($data->class_level == 'Post Graduation') {{ 'selected' }} @endif>Post Graduation</option>
                </select>
            </div>
        </div>
