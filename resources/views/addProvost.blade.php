...
@section('content')
    ...
    @endif
    <form action="{{ route('admin.provostCreate') }}" method="post">
        @csrf
        <div class="mt-2 space-y-2 px-[8px] lg:px-0">
            <div class="flex flex-col justify-start lg:flex-row lg:justify-around lg:w-full">
                <li class="flex w-full lg:w-[40%] lg:justify-between"><span>Hall</span><span>:</span></li>
                <select name="Hall_Name" id=""
                    ...
                    @foreach ($hall as $item)
                        <option value="{{ $item->name }}"@if (old('Hall_Name') == '{{ $item->name }}') {{ 'selected' }} @endif>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex flex-col justify-start lg:flex-row lg:justify-around lg:w-full">
                <li class="flex w-full lg:w-[40%] lg:justify-between"><span>Provost First Name</span><span>:</span></li>
                <input type="text" placeholder="Enter First Name" id="Class" name="First_Name"
                    value="{{ old('First_Name') }}"
                    class="ml-0 lg:w-[60%] lg:ml-[28px] border-2 border-[#006666] px-[2px] focus:outline-none">
            </div>
            <div class="flex flex-col justify-start lg:flex-row lg:justify-around lg:w-full">
                <li class="flex w-full lg:w-[40%] lg:justify-between"><span>Provost Last Name</span><span>:</span></li>
        ...
        <div class="mt-5 text-center">
            <input type="submit" value="Add Provost"
